...
module portaPitch(){
    ...
    slide();
    //translate([gearXPos,0,gearZPos])rotate([0,-90,0])gearRack(2);
    //translate([28,0,gearZPos])bearing();
...
translate([0,0,0])rotate([90,180,0])cPlate(0);
...
difference(){
union(){
    translate([0,-strPosX,0])rotate([90,0,-90])cPlate(1);
hull()rotate([180,0,0]){
...
//   }
    ...
    difference(){
      translate([-slide_rackPosX,0,-slotXY/2-cpDst+0.1])cylinder(d=bDiaI*1.6,h=zB);
      translate([-slide_rackPosX,0,-slotXY/2-cpDst])cylinder(d=bDiaI*1,h=100,center=true);
    }
    ...
    if(nutCut==0){
    translate([-xDst/2,yDst/2,-slotXY])cylinder(d=bDiaI*1,h=100);
    translate([-xDst/2,-yDst/2,-slotXY])cylinder(d=bDiaI*1,h=100);
      
    hull(){ 
      translate([xDst/2+adjL/2,0,-slotXY])cylinder(d=bDiaI*1,h=100);
      translate([xDst/2-adjL/2,0,-slotXY])cylinder(d=bDiaI*1,h=100);   
   }
    }
   ...
   if(nutCut==1){
      ...
      hull(){ 
        ...
        translate([xDst/2-adjL/2,0,-slotXY/2-cpDst-zA-0.5])cylinder(d=nutDia,h=zA-nutSp+0.5,$fn=6);
          ...
          hull(){ 
      translate([xDst/2+adjL/2,0,-slotXY/2-cpDst-nutSp+printSp])cylinder(d=bDiaI*1,h=100);
      translate([xDst/2-adjL/2,0,-slotXY/2-cpDst-nutSp+printSp])cylinder(d=bDiaI*1,h=100);   
   }
...
translate([0,-gearYPos,0])rotate([0,0,-90]){
    translate([-stepperH-mntGrD-stepperAMY/2,gearXPos,0])rotate([0,90,0])Nema17();
...
hull(){
translate([gearXPos-bearGearD+adjD/2,0,-0.5])cylinder(d=bDiaI, h=stepperAMY+1);
translate([gearXPos-bearGearD-adjD/2,0,-0.5])cylinder(d=bDiaI, h=stepperAMY+1);
}
hull(){
translate([gearXPos-bearGearD+adjD/2,0,stepperAMY-6])rotate([0,0,0])cylinder(d=15, h=stepperAMY,$fn=6);
translate([gearXPos-bearGearD-adjD/2,0,stepperAMY-6])rotate([0,0,0])cylinder(d=15, h=stepperAMY,$fn=6);
}
//hull(){
...
module bearSpc(){
translate([gearXPos,-gearYPos,0])rotate([0,0,-90]){
